<header class="vssl-stripe--header <?= $this->e(
  $type,
  "wrapperClasses"
) ?>"<?php
...
?>>
    <?php if (!empty($image)): ?>
    <div class="vssl-stripe--header--background" style="background-image: url('<?= $this->image(
      $image
    ) ?>');"></div>
    <?php endif; ?>
    <div class="vssl-stripe-column<?= !empty($layout) && $layout == "video-left"
      ? " video-left"
      : "" ?>">
        <?php if (!empty($hed["html"])): ?>
        <div class=vssl-stripe--header-video--text>
            <h1 class="vssl-stripe--header--hed vssl-stripe--header-video--hed"><?= $this->inline(
              $hed["html"]
            ) ?></h1>
        ...
        <div class="vssl-stripe--header-video--inset">
            <div class="vssl-stripe--header-video--ui">
                ...
                <video loop autoplay muted playsinline
                    <?= !empty($inset)
                      ? 'poster="' .
                        $this->image($inset, $image_style ?? null) .
                        '"'
                      : "" ?>
                >
                ...
                <a class="vssl-stripe--header-video--playbtn-wrap" href="<?= $videoUrl ??
                  "#video--" . $id ?>" target="_blank">
            ...
            <div
                ...
                data-video-embed="<?= htmlspecialchars(
                  json_encode(["code" => $videoEmbed]),
                  ENT_QUOTES
                ) ?>"
            ></div>
            <?php endif; ?>
