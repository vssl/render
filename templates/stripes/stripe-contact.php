...
echo !empty($headshot) ? ' data-has-headshot="true"' : "";
        ...
        <div class="vssl-stripe--contact--card vssl-stripe--card">
            ...
            <div class="vssl-stripe--contact--headshot">
                <img
                    src="<?= $this->image(
                      $headshot,
                      !empty($image_style) ? $image_style : "headshot"
                    ) ?>"
            ...
            <div class="vssl-stripe--contact--info">
                ...
                <h3 class="vssl-stripe--contact--name"><?= $this->inline(
                  $name["html"]
                ) ?></h3>
                <?php endif; ?>

                <?php if (!empty($jobtitle["html"])): ?>
                <div class="vssl-stripe--contact--jobtitle"><?= $this->inline(
                  $jobtitle["html"]
                ) ?></div>
                <?php endif; ?>
                ...
                <div class="vssl-stripe--contact--email">
                    <a href="mailto:<?= strip_tags(
                      $email["html"]
                    ) ?>"><?= strip_tags($email["html"]) ?></a>
                </div>
                <?php endif; ?>

                <?php if (!empty($phone["html"])): ?>
                <div class="vssl-stripe--contact--phone"><?= $this->inline(
                  $phone["html"]
                ) ?></div>
                <?php endif; ?>
            </div>
